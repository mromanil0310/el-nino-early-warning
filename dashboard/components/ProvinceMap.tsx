  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      <h2 className="text-sm font-semibold text-gray-700 mb-1">
        Risk map — {points.length} {points.length === 1 ? 'province' : 'provinces'} scored
      </h2>
      {onSelect && (
        <p className="text-xs text-gray-500 mb-3">Click a province to see its details below.</p>
      )}
      <div className="flex flex-col sm:flex-row gap-4">
        <svg
          ...
          role="img"
          aria-label="Map of provinces colored by El Niño risk level"
        >
          {points.map((p) => (
            <circle
              key={p.id}
              ...
              aria-label={`${p.name}: ${p.level} risk, score ${Math.round(p.score)}`}
