...
**Remaining before public launch:** deploy the inbound webhook (with `SEMAPHORE_WEBHOOK_SECRET` from ELN-025), set up Semaphore.ph for real SMS delivery, then let the pipeline run for real (`dry_run=false`) or on its Monday 6am schedule.
