...
**Calibration shift to review (domain judgment):** using the anomaly signal instead of a flat category raises the driest provinces slightly and lowers the mildest. At reproductive stage: Nueva Ecija ~83 (was 75, still High); Laguna/Batangas ~54 and Quezon ~48 move Low→**Medium** (their −5…−10% anomaly now counts). Tune the slopes in `outlook.probabilities_from_anomaly` if a different calibration is preferred.
